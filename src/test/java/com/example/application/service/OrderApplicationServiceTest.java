...
public class OrderApplicationServiceTest {
  ...
  void getOrders() {
    ...
    Order order1 = Order.builder().orderId("1").productId("product1").quantity(3)
            .status("submit").createTime(createTime).updateTime(createTime).build();

    Order order2 = Order.builder().orderId("1").productId("product2").quantity(2)
            .status("submit").createTime(createTime).updateTime(createTime).build();

    Order order3 = Order.builder().orderId("2").productId("product1").quantity(5)
            .status("submit").createTime(createTime).updateTime(createTime).build();

    Product product1 = Product.builder().id("product1").name("book").price(BigDecimal.valueOf(20)).build();
    Product product2 = Product.builder().id("product2").name("cat").price(BigDecimal.valueOf(200)).build();
